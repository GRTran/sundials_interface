...
subroutine shampODE ( ode, neqn, y, t, tout, relerr, abserr, iflag, work, iwork )
...
!
  ...
  CLASS(ode_test), INTENT( INOUT )                  ::    ode
  ...
  call de (ode, neqn, y, t, tout, relerr, abserr, iflag, work(iyy), &
...
subroutine de ( ode, neqn, y, t, tout, relerr, abserr, iflag, yy, wt, p, yp, &
...
!
    if ( isn <= 0 .and. abs ( tout - x ) < fouru * abs ( x ) ) then
      h = tout - x
      call ode%calculate_gradient(x, yy, yp )
...
!
    ...
    call step ( x, yy, ode, neqn, h, eps, wt, start, &
...
subroutine step ( x, y, ode, neqn, h, eps, wt, start, hold, k, kold, crash, &
...
!
  if ( start ) then
    call ode%calculate_gradient(x, y, yp )
    !write(*,*) yp
